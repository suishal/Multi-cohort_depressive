...
library(clusterSim)
setwd("/OneDrive - connect.hku.hk/project/systematic_review_and_meta_analysis/Analysis/Final/Ref77/")
...
##metadata
metadata <- read.table("/OneDrive - connect.hku.hk/project/systematic_review_and_meta_analysis/DataDownload/Ref77/12382_20200402-110213.txt",sep="\t",head=T,comment.char = "",check.names = F)
rownames(metadata) <- metadata$host_subject_id
metadata <- metadata[,c(2,4,51,21)]
head(metadata)
metadata$thispapercatolog <- 0
metadata$thispapercatolog[metadata$hamilton_depression >7] <- 1
...
#observe otu
obt <- read.table("dada2/observed_otus_vector.qza/50760af3-bd44-4c64-a142-90c6d14ac5f5/data/alpha-diversity.tsv")

#chao1
chao1.d <- read.table("dada2/chao1_alpha_diverisyt.qza/a8c56866-9986-4b1a-a5ce-c0faace20007/data/alpha-diversity.tsv")

#shannon
shannon <- read.table("dada2/shannon_vector.qza/2917f504-4d2a-46e9-a709-f10bdcb8e4ea/data/alpha-diversity.tsv")

#simpson
simpson <- read.table("dada2/simpson_alpha_diverisyt.qza/37549745-d634-4622-9027-e3e187d3d179/data/alpha-diversity.tsv")

##2.taxonomy
kingdom <- read.csv("dada2/taxarare-bar-plots.qzv/74a5e344-324b-41cf-80fe-b70e34a02378/data/level-1.csv")
phylum <- read.csv("dada2/taxarare-bar-plots.qzv/74a5e344-324b-41cf-80fe-b70e34a02378/data/level-2.csv")
class <- read.csv("dada2/taxarare-bar-plots.qzv/74a5e344-324b-41cf-80fe-b70e34a02378/data/level-3.csv")
ord <- read.csv("dada2/taxarare-bar-plots.qzv/74a5e344-324b-41cf-80fe-b70e34a02378/data/level-4.csv")
family <- read.csv("dada2/taxarare-bar-plots.qzv/74a5e344-324b-41cf-80fe-b70e34a02378/data/level-5.csv")
genus <- read.csv("dada2/taxarare-bar-plots.qzv/74a5e344-324b-41cf-80fe-b70e34a02378/data/level-6.csv")
species <- read.csv("dada2/taxarare-bar-plots.qzv/74a5e344-324b-41cf-80fe-b70e34a02378/data/level-7.csv")
...
write.table(t.res,"Ref77_dada2_alpha.diversity.t.test.result.txt",sep="\t",quote = F,col.names = T,row.names = T)
write.table(wil.res,"Ref77_dada2_alpha.diversity.wilcoxon.test.result.txt",sep="\t",quote = F,col.names = T,row.names = T)


###########alpha divesity odd ratio
alpha.odd <- alpha[,c(1:4,5,6,7,9)]
colnames(alpha.odd)[5:8]<-c("Age","BMI","Sex","Depression")
odd.crude.res <- c()
odd.adjust.res <- c()
library(survival)
for(i in 1:4){
  ...
  #adjust
  data <- cbind(alpha.odd[,i,drop=F],alpha.odd[,c("Depression","Age","BMI","Sex")])
  colnames(data)[1] <- "alpha"
  reg.adjust <- glm(Depression ~alpha+Age+BMI+Sex ,family=binomial, data=data)
  odd.adjust.res <- rbind(odd.adjust.res,
                         exp(cbind(coef(reg.adjust), confint(reg.adjust)))[2,])
  ...
  data$H.alpha <- 1
  data$H.alpha[data$alpha < med.alpha] <-0
  reg.adjust <- glm(Depression ~H.alpha+Age+BMI+Sex ,family=binomial, data=data)
  odd.adjust.res <- rbind(odd.adjust.res,
                          round(exp(cbind(coef(reg.adjust), confint(reg.adjust))),3)[2,])
...
write.table(odd.crude.res,"Ref77_dada_alpha.diversity.oR.result.txt",sep="\t",quote = F,col.names = T,row.names = T)
write.table(odd.adjust.res,"Ref77_dada_alpha.diversity.OR_adjust.result.txt",sep="\t",quote = F,col.names = T,row.names = T)
...
write.table(t.res,"Ref77.dada2_phylum.t.test.result.txt",sep="\t",quote = F,col.names = T,row.names = T)
write.table(wil.res,"Ref77.dada2_phylum.wilcoxon.test.result.txt",sep="\t",quote = F,col.names = T,row.names = T)
...
phylum.transf <- matrix(data=NA,nrow = 130,ncol = length(phy.p))
...
colnames(genus.d)
sum(apply(genus,2,function(x){sum(x!=0)})==130)
###genus transfer
names(sort(apply(genus.d[,c(1:dim(genus)[2])],2,function(x){sum(x!=0)}),decreasing = T))[1:20]
id <- which(apply(genus.d[,c(1:dim(genus)[2])],2,function(x){sum(x!=0)})==130)

df <- matrix(data=NA,nrow = 130,ncol = length(id))
p<-c()
...
rownames(wil.res) <- rownames(t.res) <- colnames(genus.d)[1:324]
...
write.table(t.res,"Ref77_dada2.genus.t.test.result.txt",sep="\t",quote = F,col.names = T,row.names = T)
write.table(wil.res,"Ref77_dada2.genus.wilcoxon.test.result.txt",sep="\t",quote = F,col.names = T,row.names = T)
...
lefse.genus <- t(genus.d[,c(329,1:324)])
...
write.table(lefse.genus,"Ref77.lefse.genus.dada.txt",sep="\t",quote = F,col.names = F)


###compared with systematic review
library(metamicrobiomeR) 

taz.nor.com <- cbind(metadata[,c("age","bmi","sex","thispapercatolog")],All.tax)
genus.nor.com <- cbind(metadata[,c("age","bmi","sex","thispapercatolog")],genus.nor)
...
colnames(taz.nor.com)[1:4] <- c("Age","BMI","Gender","Depression")
colnames(genus.nor.com)[1:4] <- c("Age","BMI","Gender","Depression")

taxacom.tax<-taxa.compare(taxtab=taz.nor.com,propmed.rel="gamlss",comvar="Depression",adjustvar=c("Age","BMI","Gender"),longitudinal="no",p.adjust.method="fdr")

taxacom.genus<-taxa.compare(taxtab=genus.nor.com,propmed.rel="gamlss",comvar="Depression",adjustvar=c("Age","BMI","Gender"),longitudinal="no",p.adjust.method="fdr")

write.table(taxacom.genus,"Ref77.metaR.genus.dada2.result.txt",sep="\t",quote = F,row.names = F,col.names = T)
write.table(taxacom.tax,"Ref77.metaR.tax.result.dada2.txt",sep="\t",quote = F,row.names = F,col.names = T)

taxcomtab.show(taxcomtab=taxacom.genus, tax.lev="l6",tax.select="none",showvar="Depression1_depression",p.cutoff=0.1)
...
rownames(All.tax) <- paste("Ref77.",rownames(All.tax),sep="")
write.table(t(All.tax),"Ref77.abd.dada2.MMUPHin.txt",sep="\t",quote = F)
metadata.MMUPHin <- metadata
colnames(metadata.MMUPHin) <- c("Age","BMI","Sex","depression_score","Depression")
rownames(metadata.MMUPHin) <- paste("Ref77.",rownames(metadata.MMUPHin),sep="")
metadata.MMUPHin$subjectID <- "Ref77"
write.table(metadata.MMUPHin,"Ref77.meta.dada2.MMUPHin.txt",sep="\t",quote = F)
